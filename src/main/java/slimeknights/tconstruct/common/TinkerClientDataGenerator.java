...
public class TinkerClientDataGenerator {
  ...
  static void gatherData(GatherDataEvent.Client event) {
    ...
    // assets/tconstruct/materials/... render info json
    pack.addProvider(output -> new MaterialRenderInfoProvider(output, materialSprites, resourceManager));
    // assets/tconstruct/tinkering/generator_part_textures.json (consumed by the in-game part texture command)
    ...
    // debug palette dump (excluded from the built jar via assets/tconstruct/debug)
